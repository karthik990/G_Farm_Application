...
public class PathInterpolatorCompat implements Interpolator {
    ...
    private void initPath(Path path) {
        ...
        if (min > 0) {
            this.f63mX = new float[min];
            this.f64mY = new float[min];
            float[] fArr = new float[2];
            for (int i2 = 0; i2 < min; i2++) {
                pathMeasure.getPosTan((((float) i2) * length) / ((float) (min - 1)), fArr, null);
                this.f63mX[i2] = fArr[0];
                this.f64mY[i2] = fArr[1];
            }
            if (((double) Math.abs(this.f63mX[0])) <= 1.0E-5d && ((double) Math.abs(this.f64mY[0])) <= 1.0E-5d) {
                int i3 = min - 1;
                if (((double) Math.abs(this.f63mX[i3] - 1.0f)) <= 1.0E-5d && ((double) Math.abs(this.f64mY[i3] - 1.0f)) <= 1.0E-5d) {
                    int i4 = 0;
                    float f = 0.0f;
                    while (i < min) {
                        float[] fArr2 = this.f63mX;
                        int i5 = i4 + 1;
                        float f2 = fArr2[i4];
                        if (f2 >= f) {
                            fArr2[i] = f2;
            ...
            sb2.append(this.f63mX[0]);
            String str = ",";
            sb2.append(str);
            sb2.append(this.f64mY[0]);
            sb2.append(" end:");
            int i6 = min - 1;
            sb2.append(this.f63mX[i6]);
            sb2.append(str);
            sb2.append(this.f64mY[i6]);
