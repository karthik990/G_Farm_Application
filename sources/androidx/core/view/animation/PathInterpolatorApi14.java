...
class PathInterpolatorApi14 implements Interpolator {
    ...
    /* renamed from: mX */
    private final float[] f52mX;

    /* renamed from: mY */
    private final float[] f53mY;

    PathInterpolatorApi14(Path path) {
        PathMeasure pathMeasure = new PathMeasure(path, false);
        float length = pathMeasure.getLength();
        int i = ((int) (length / PRECISION)) + 1;
        this.f52mX = new float[i];
        this.f53mY = new float[i];
        float[] fArr = new float[2];
        for (int i2 = 0; i2 < i; i2++) {
            pathMeasure.getPosTan((((float) i2) * length) / ((float) (i - 1)), fArr, null);
            this.f52mX[i2] = fArr[0];
            this.f53mY[i2] = fArr[1];
        }
    }
    ...
    public float getInterpolation(float f) {
        ...
        int i = 0;
        int length = this.f52mX.length - 1;
        while (length - i > 1) {
            int i2 = (i + length) / 2;
            if (f < this.f52mX[i2]) {
                length = i2;
        ...
        float[] fArr = this.f52mX;
        float f2 = fArr[length] - fArr[i];
        if (f2 == 0.0f) {
            return this.f53mY[i];
        }
        float f3 = (f - fArr[i]) / f2;
        float[] fArr2 = this.f53mY;
        float f4 = fArr2[i];
