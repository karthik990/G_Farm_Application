...
public final class BrowserServiceFileProvider extends FileProvider {
                ...
                for (File file2 : file.listFiles()) {
                    if (isImageFile(file2)) {
                        if (file2.lastModified() < currentTimeMillis && !file2.delete()) {
                            String str = BrowserServiceFileProvider.TAG;
        ...
        private void saveFileBlocking(File file) {
            ...
            if (VERSION.SDK_INT >= 22) {
                ...
                try {
                    ...
                    try {
                        this.mBitmap.compress(CompressFormat.PNG, 100, fileOutputStream);
                ...
                try {
                    ...
                    this.mBitmap.compress(CompressFormat.PNG, 100, fileOutputStream2);
    ...
    public static ResolvableFuture<Uri> saveBitmap(Context context, Bitmap bitmap, String str, int i) {
        ...
        FileSaveTask fileSaveTask = new FileSaveTask(context, sb2, bitmap, generateUri, create);
        fileSaveTask.executeOnExecutor(AsyncTask.THREAD_POOL_EXECUTOR, new String[0]);
        return create;
    }

    private static Uri generateUri(Context context, String str) {
        StringBuilder sb = new StringBuilder();
        sb.append(FILE_SUB_DIR_NAME);
        sb.append(str);
        ...
        sb3.append(AUTHORITY_SUFFIX);
        return scheme.authority(sb3.toString()).path(sb2).build();
    }
    ...
    public static void grantReadPermission(Intent intent, List<Uri> list, Context context) {
        if (list != null && list.size() != 0) {
            ...
            intent.addFlags(1);
            ClipData newUri = ClipData.newUri(contentResolver, CLIP_DATA_LABEL, (Uri) list.get(0));
            for (int i = 1; i < list.size(); i++) {
                newUri.addItem(new Item((Uri) list.get(i)));
            }
