...
final class ExecutorCallAdapterFactory extends Factory {
    ...
    static final class ExecutorCallbackCall<T> implements Call<T> {
        ...
        public void enqueue(final Callback<T> callback) {
            C2447Utils.checkNotNull(callback, "callback == null");
            this.delegate.enqueue(new Callback<T>() {
                public void onResponse(Call<T> call, final Response<T> response) {
                    ExecutorCallbackCall.this.callbackExecutor.execute(new Runnable() {
                        public void run() {
                            if (ExecutorCallbackCall.this.delegate.isCanceled()) {
                                callback.onFailure(ExecutorCallbackCall.this, new IOException(ECommerceConstant.CANCELED));
                            } else {
    ...
    public CallAdapter<?, ?> get(Type type, Annotation[] annotationArr, Retrofit retrofit) {
        ...
        final Type callResponseType = C2447Utils.getCallResponseType(type);
