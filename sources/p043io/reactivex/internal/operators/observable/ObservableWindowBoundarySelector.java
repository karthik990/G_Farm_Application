...
public final class ObservableWindowBoundarySelector<T, B, V> extends AbstractObservableWithUpstream<T, Observable<T>> {
    ...
    static final class OperatorWindowBoundaryCloseObserver<T, V> extends DisposableObserver<V> {
        ...
        /* renamed from: w */
        final UnicastSubject<T> f4087w;

        OperatorWindowBoundaryCloseObserver(WindowBoundaryMainObserver<T, ?, V> windowBoundaryMainObserver, UnicastSubject<T> unicastSubject) {
            this.parent = windowBoundaryMainObserver;
            this.f4087w = unicastSubject;
        }
    ...
    static final class WindowBoundaryMainObserver<T, B, V> extends QueueDrainObserver<T, Object, Observable<T>> implements Disposable {
        ...
        /* renamed from: ws */
        final List<UnicastSubject<T>> f4089ws;
        ...
        WindowBoundaryMainObserver(Observer<? super Observable<T>> observer, ObservableSource<B> observableSource, Function<? super B, ? extends ObservableSource<V>> function, int i) {
            ...
            this.f4089ws = new ArrayList();
            this.windows.lazySet(1);
        }

        public void onSubscribe(Disposable disposable) {
            if (DisposableHelper.validate(this.f4088s, disposable)) {
                this.f4088s = disposable;
        ...
        public void onNext(T t) {
            if (fastEnter()) {
                for (UnicastSubject onNext : this.f4089ws) {
                    onNext.onNext(t);
                }
                if (leave(-1) == 0) {
                    return;
        ...
        public void drainLoop() {
            ...
            List<UnicastSubject<T>> list = this.f4089ws;
            int i = 1;
            while (true) {
                boolean z = this.done;
                ...
                if (z && z2) {
                    ...
                    if (windowOperation.f4090w != null) {
                        if (list.remove(windowOperation.f4090w)) {
                            windowOperation.f4090w.onComplete();
                            if (this.windows.decrementAndGet() == 0) {
                                disposeBoundary();
                                return;
                            }
        ...
        public void close(OperatorWindowBoundaryCloseObserver<T, V> operatorWindowBoundaryCloseObserver) {
            ...
            this.queue.offer(new WindowOperation(operatorWindowBoundaryCloseObserver.f4087w, null));
            if (enter()) {
                drainLoop();
            }
        }
    ...
    static final class WindowOperation<T, B> {
        ...
        /* renamed from: w */
        final UnicastSubject<T> f4090w;

        WindowOperation(UnicastSubject<T> unicastSubject, B b) {
            this.f4090w = unicastSubject;
            this.open = b;
        }
    }
