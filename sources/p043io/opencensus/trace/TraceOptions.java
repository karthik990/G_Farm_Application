...
public final class TraceOptions {
    ...
    public static TraceOptions fromBytes(byte[] bArr) {
        C5887Utils.checkNotNull(bArr, "buffer");
        C5887Utils.checkArgument(bArr.length == 1, "Invalid size: expected %s, got %s", Integer.valueOf(1), Integer.valueOf(bArr.length));
        return fromByte(bArr[0]);
    }

    @Deprecated
    public static TraceOptions fromBytes(byte[] bArr, int i) {
        C5887Utils.checkIndex(i, bArr.length);
