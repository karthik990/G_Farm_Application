...
public class ChatBlockedUserListActivity extends AveActivity {
    ...
    private void getBannedUsers() {
        FirebaseDatabase.getInstance().getReference().child(ChatConstants.ARG_USERS).child(FirebaseAuth.getInstance().getCurrentUser().getUid()).child(ChatConstants.ARG_USERS_BLOCKED_LIST).addListenerForSingleValueEvent(new ValueEventListener() {
            public void onCancelled(DatabaseError databaseError) {
            }
            ...
            public void onDataChange(DataSnapshot dataSnapshot) {
                ...
                if (hashMap != null) {
                    for (Entry key : hashMap.entrySet()) {
                        ChatBlockedUserListActivity.this.getUserFromFirebase((String) key.getKey());
                    }
                }
            }
        });
