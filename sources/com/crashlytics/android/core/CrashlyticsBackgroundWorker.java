...
class CrashlyticsBackgroundWorker {
    ...
    public <T> T submitAndWait(Callable<T> callable) {
        ...
        try {
            ...
            Fabric.getLogger().mo64074d(str, "Executor is shut down because we're handling a fatal crash.");
            return null;
        } catch (Exception e) {
            Fabric.getLogger().mo64077e(str, "Failed to execute task.", e);
            return null;
        }
    }
            ...
            return this.executorService.submit(new Runnable() {
                public void run() {
                    try {
                        ...
                        Fabric.getLogger().mo64077e(CrashlyticsCore.TAG, "Failed to execute task.", e);
                    }
                }
            });
        } catch (RejectedExecutionException unused) {
            Fabric.getLogger().mo64074d(CrashlyticsCore.TAG, "Executor is shut down because we're handling a fatal crash.");
            return null;
        }
    }
