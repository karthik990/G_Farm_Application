...
import p101me.zhanghai.android.materialprogressbar.HorizontalProgressDrawable;
import p101me.zhanghai.android.materialprogressbar.IndeterminateCircularProgressDrawable;
import p101me.zhanghai.android.materialprogressbar.IndeterminateHorizontalProgressDrawable;

class DialogInit {
    DialogInit() {
    }

    static int getTheme(Builder builder) {
        boolean resolveBoolean = DialogUtils.resolveBoolean(builder.context, C0829R.attr.md_dark_theme, builder.theme == Theme.DARK);
        builder.theme = resolveBoolean ? Theme.DARK : Theme.LIGHT;
        return resolveBoolean ? C0829R.C0834style.MD_Dark : C0829R.C0834style.MD_Light;
    }

    static int getInflateLayout(Builder builder) {
        if (builder.customView != null) {
            return C0829R.layout.md_dialog_custom;
        }
        if (builder.items == null && builder.adapter == null) {
            if (builder.progress > -2) {
                return C0829R.layout.md_dialog_progress;
            }
            if (builder.indeterminateProgress) {
                if (builder.indeterminateIsHorizontalProgress) {
                    return C0829R.layout.md_dialog_progress_indeterminate_horizontal;
                }
                return C0829R.layout.md_dialog_progress_indeterminate;
            } else if (builder.inputCallback != null) {
                if (builder.checkBoxPrompt != null) {
                    return C0829R.layout.md_dialog_input_check;
                }
                return C0829R.layout.md_dialog_input;
            } else if (builder.checkBoxPrompt != null) {
                return C0829R.layout.md_dialog_basic_check;
            } else {
                return C0829R.layout.md_dialog_basic;
            }
        } else if (builder.checkBoxPrompt != null) {
            return C0829R.layout.md_dialog_list_check;
        } else {
            return C0829R.layout.md_dialog_list;
        }
    }
    ...
    public static void init(MaterialDialog materialDialog) {
        ...
        if (builder.backgroundColor == 0) {
            builder.backgroundColor = DialogUtils.resolveColor(builder.context, C0829R.attr.md_background_color, DialogUtils.resolveColor(materialDialog.getContext(), C0829R.attr.colorBackgroundFloating));
        }
        if (builder.backgroundColor != 0) {
            GradientDrawable gradientDrawable = new GradientDrawable();
            gradientDrawable.setCornerRadius(builder.context.getResources().getDimension(C0829R.dimen.md_bg_corner_radius));
            gradientDrawable.setColor(builder.backgroundColor);
            materialDialog.getWindow().setBackgroundDrawable(gradientDrawable);
        }
        if (!builder.positiveColorSet) {
            builder.positiveColor = DialogUtils.resolveActionTextColorStateList(builder.context, C0829R.attr.md_positive_color, builder.positiveColor);
        }
        if (!builder.neutralColorSet) {
            builder.neutralColor = DialogUtils.resolveActionTextColorStateList(builder.context, C0829R.attr.md_neutral_color, builder.neutralColor);
        }
        if (!builder.negativeColorSet) {
            builder.negativeColor = DialogUtils.resolveActionTextColorStateList(builder.context, C0829R.attr.md_negative_color, builder.negativeColor);
        }
        if (!builder.widgetColorSet) {
            builder.widgetColor = DialogUtils.resolveColor(builder.context, C0829R.attr.md_widget_color, builder.widgetColor);
        }
        if (!builder.titleColorSet) {
            builder.titleColor = DialogUtils.resolveColor(builder.context, C0829R.attr.md_title_color, DialogUtils.resolveColor(materialDialog.getContext(), 16842806));
        }
        if (!builder.contentColorSet) {
            builder.contentColor = DialogUtils.resolveColor(builder.context, C0829R.attr.md_content_color, DialogUtils.resolveColor(materialDialog.getContext(), 16842808));
        }
        if (!builder.itemColorSet) {
            builder.itemColor = DialogUtils.resolveColor(builder.context, C0829R.attr.md_item_color, builder.contentColor);
        }
        materialDialog.title = (TextView) materialDialog.view.findViewById(C0829R.C0832id.md_title);
        materialDialog.icon = (ImageView) materialDialog.view.findViewById(C0829R.C0832id.md_icon);
        materialDialog.titleFrame = materialDialog.view.findViewById(C0829R.C0832id.md_titleFrame);
        materialDialog.content = (TextView) materialDialog.view.findViewById(C0829R.C0832id.md_content);
        materialDialog.recyclerView = (RecyclerView) materialDialog.view.findViewById(C0829R.C0832id.md_contentRecyclerView);
        materialDialog.checkBoxPrompt = (CheckBox) materialDialog.view.findViewById(C0829R.C0832id.md_promptCheckbox);
        materialDialog.positiveButton = (MDButton) materialDialog.view.findViewById(C0829R.C0832id.md_buttonDefaultPositive);
        materialDialog.neutralButton = (MDButton) materialDialog.view.findViewById(C0829R.C0832id.md_buttonDefaultNeutral);
        materialDialog.negativeButton = (MDButton) materialDialog.view.findViewById(C0829R.C0832id.md_buttonDefaultNegative);
        ...
        if (builder.icon != null) {
            ...
            Drawable resolveDrawable = DialogUtils.resolveDrawable(builder.context, C0829R.attr.md_icon);
        ...
        if (i == -1) {
            i = DialogUtils.resolveDimension(builder.context, C0829R.attr.md_icon_max_size);
        }
        if (builder.limitIconToDefaultSize || DialogUtils.resolveBoolean(builder.context, C0829R.attr.md_icon_limit_icon_to_default_size)) {
            i = builder.context.getResources().getDimensionPixelSize(C0829R.dimen.md_icon_max_size);
        }
        if (i > -1) {
        ...
        if (!builder.dividerColorSet) {
            builder.dividerColor = DialogUtils.resolveColor(builder.context, C0829R.attr.md_divider_color, DialogUtils.resolveColor(materialDialog.getContext(), C0829R.attr.md_divider));
        }
        ...
        if (VERSION.SDK_INT >= 14) {
            ...
            if (z) {
                z = DialogUtils.resolveBoolean(builder.context, C0829R.attr.textAllCaps, true);
            }
        } else {
            z = DialogUtils.resolveBoolean(builder.context, C0829R.attr.textAllCaps, true);
        }
        ...
        if (builder.customView != null) {
            ((MDRootLayout) materialDialog.view.findViewById(C0829R.C0832id.md_root)).noTitleNoPadding();
            FrameLayout frameLayout = (FrameLayout) materialDialog.view.findViewById(C0829R.C0832id.md_customViewFrame);
            ...
            if (builder.wrapCustomViewInScroll) {
                ...
                int dimensionPixelSize = resources.getDimensionPixelSize(C0829R.dimen.md_dialog_frame_margin);
                ScrollView scrollView = new ScrollView(materialDialog.getContext());
                int dimensionPixelSize2 = resources.getDimensionPixelSize(C0829R.dimen.md_content_padding_top);
                int dimensionPixelSize3 = resources.getDimensionPixelSize(C0829R.dimen.md_content_padding_bottom);
        ...
        int i2 = point.x;
        int i3 = point.y;
        int dimensionPixelSize4 = builder.context.getResources().getDimensionPixelSize(C0829R.dimen.md_dialog_vertical_margin);
        int dimensionPixelSize5 = builder.context.getResources().getDimensionPixelSize(C0829R.dimen.md_dialog_horizontal_margin);
        int dimensionPixelSize6 = builder.context.getResources().getDimensionPixelSize(C0829R.dimen.md_dialog_max_width);
        int i4 = i2 - (dimensionPixelSize5 * 2);
        materialDialog.view.setMaxHeight(i3 - (dimensionPixelSize4 * 2));
        WindowManager.LayoutParams layoutParams = new WindowManager.LayoutParams();
        layoutParams.copyFrom(materialDialog.getWindow().getAttributes());
        layoutParams.width = Math.min(dimensionPixelSize6, i4);
        ...
        if (builder.indeterminateProgress || builder.progress > -2) {
            ...
            if (materialDialog.progressBar != null) {
                ...
                if (!builder.indeterminateProgress || builder.indeterminateIsHorizontalProgress) {
                    ...
                    materialDialog.progressLabel = (TextView) materialDialog.view.findViewById(C0829R.C0832id.md_label);
                    ...
                    materialDialog.progressMinMax = (TextView) materialDialog.view.findViewById(C0829R.C0832id.md_minMax);
                    if (materialDialog.progressMinMax != null) {
                        ...
                        if (builder.showMinMax) {
                            ...
                            materialDialog.progressMinMax.setText(String.format(builder.progressNumberFormat, new Object[]{Integer.valueOf(0), Integer.valueOf(builder.progressMax)}));
                            MarginLayoutParams marginLayoutParams = (MarginLayoutParams) materialDialog.progressBar.getLayoutParams();
    ...
    private static void setupInputDialog(MaterialDialog materialDialog) {
        ...
        if (materialDialog.input != null) {
            ...
            materialDialog.inputMinMax = (TextView) materialDialog.view.findViewById(C0829R.C0832id.md_minMax);
