...
class SSOAuthHandler extends AuthHandler {
    ...
    private boolean startAuthActivityForResult(Activity activity) {
        ...
        if (availableSSOPackage == null) {
            Twitter.getLogger().mo20820e(str, "SSO app signature check failed", null);
            return false;
        }
        ...
        if (!IntentUtils.isActivityAvailable(activity, component)) {
            Twitter.getLogger().mo20820e(str, "SSO auth activity not found", null);
            return false;
        }
        ...
        try {
            ...
            Twitter.getLogger().mo20820e(str, "SSO exception occurred", e);
            return false;
        }
    }

    public static String availableSSOPackage(PackageManager packageManager) {
        String str = TWITTER_PACKAGE_NAME;
        if (checkAppSignature(packageManager, str, TWITTER_SIGNATURE)) {
            return str;
        }
        String str2 = DOGFOOD_PACKAGE_NAME;
        if (checkAppSignature(packageManager, str2, DOGFOOD_SIGNATURE)) {
            return str2;
        }
        return null;
    }
