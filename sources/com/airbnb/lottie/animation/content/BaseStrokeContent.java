...
public abstract class BaseStrokeContent implements AnimationListener, KeyPathElementContent, DrawingContent {
    ...
    /* renamed from: pm */
    private final PathMeasure f77pm = new PathMeasure();
    ...
    BaseStrokeContent(LottieDrawable lottieDrawable2, BaseLayer baseLayer, Cap cap, Join join, AnimatableIntegerValue animatableIntegerValue, AnimatableFloatValue animatableFloatValue, List<AnimatableFloatValue> list, AnimatableFloatValue animatableFloatValue2) {
    ...
    public void draw(Canvas canvas, Matrix matrix, int i) {
        ...
        this.paint.setStrokeWidth(((Float) this.widthAnimation.getValue()).floatValue() * C0894Utils.getScale(matrix));
    ...
    private void applyTrimPath(Canvas canvas, PathGroup pathGroup, Matrix matrix) {
        ...
        this.f77pm.setPath(this.path, false);
        float length = this.f77pm.getLength();
        while (this.f77pm.nextContour()) {
            length += this.f77pm.getLength();
        }
        ...
        for (int size2 = pathGroup.paths.size() - 1; size2 >= 0; size2--) {
            ...
            this.f77pm.setPath(this.trimPathPath, false);
            float length2 = this.f77pm.getLength();
            float f2 = 1.0f;
            if (floatValue3 > length) {
                float f3 = floatValue3 - length;
                if (f3 < f + length2 && f < f3) {
                    C0894Utils.applyTrimPathIfNeeded(this.trimPathPath, floatValue2 > length ? (floatValue2 - length) / length2 : 0.0f, Math.min(f3 / length2, 1.0f), 0.0f);
            ...
            if (f4 >= floatValue2 && f <= floatValue3) {
                if (f4 > floatValue3 || floatValue2 >= f) {
                    ...
                    C0894Utils.applyTrimPathIfNeeded(this.trimPathPath, f5, f2, 0.0f);
                    canvas.drawPath(this.trimPathPath, this.paint);
                } else {
                    canvas.drawPath(this.trimPathPath, this.paint);
                }
    ...
    public void getBounds(RectF rectF, Matrix matrix) {
        ...
        float floatValue = ((Float) this.widthAnimation.getValue()).floatValue();
    ...
    private void applyDashPatternIfNeeded(Matrix matrix) {
        ...
        float scale = C0894Utils.getScale(matrix);
