...
class BitmapHunter implements Runnable {
    ...
    static Bitmap decodeStream(InputStream inputStream, Request request) throws IOException {
        ...
        boolean isWebPFile = C4694Utils.isWebPFile(markableInputStream);
        markableInputStream.reset(savePosition);
        if (isWebPFile) {
            byte[] byteArray = C4694Utils.toByteArray(markableInputStream);
    ...
    public void run() {
        ...
        try {
            ...
            if (this.picasso.loggingEnabled) {
                C4694Utils.log("Hunter", "executing", C4694Utils.getLogIdsForHunter(this));
            }
            this.result = hunt();
            if (this.result == null) {
    ...
    public Bitmap hunt() throws IOException {
        ...
        if (MemoryPolicy.shouldReadFromMemoryCache(this.memoryPolicy)) {
            ...
            if (bitmap != null) {
                ...
                if (this.picasso.loggingEnabled) {
                    C4694Utils.log("Hunter", "decoded", this.data.logId(), "from cache");
                }
                return bitmap;
            }
        } else {
        ...
        if (load != null) {
            ...
            if (bitmap == null) {
                ...
                try {
                    ...
                    C4694Utils.closeQuietly(stream);
                    bitmap = decodeStream;
                } catch (Throwable th) {
                    C4694Utils.closeQuietly(stream);
                    throw th;
        ...
        if (bitmap != null) {
            if (this.picasso.loggingEnabled) {
                C4694Utils.log("Hunter", "decoded", this.data.logId());
            }
            ...
            if (this.data.needsTransformation() || this.exifRotation != 0) {
                synchronized (DECODE_LOCK) {
                    if (this.data.needsMatrixTransform() || this.exifRotation != 0) {
                        ...
                        if (this.picasso.loggingEnabled) {
                            C4694Utils.log("Hunter", "transformed", this.data.logId());
                        }
                    }
                    if (this.data.hasCustomTransformations()) {
                        bitmap = applyCustomTransformations(this.data.transformations, bitmap);
                        if (this.picasso.loggingEnabled) {
                            C4694Utils.log("Hunter", "transformed", this.data.logId(), "from custom transformations");
                        }
                    }
                }
                if (bitmap != null) {
        ...
        if (this.action == null) {
            ...
            if (z) {
                ...
                if (list == null || list.isEmpty()) {
                    C4694Utils.log(str3, str2, request.logId(), "to empty hunter");
                } else {
                    C4694Utils.log(str3, str2, request.logId(), C4694Utils.getLogIdsForHunter(this, str));
                }
            }
            return;
        }
    ...
    public void detach(Action action2) {
        ...
        if (this.picasso.loggingEnabled) {
            C4694Utils.log("Hunter", "removed", action2.request.logId(), C4694Utils.getLogIdsForHunter(this, "from "));
        }
    }
