...
public final class HttpEngine {
    ...
    private Response unzip(Response response) throws IOException {
        if (this.transparentGzip) {
            ...
            if (HttpRequest.ENCODING_GZIP.equalsIgnoreCase(this.userResponse.header(str))) {
    ...
    private Request networkRequest(Request request) throws IOException {
        ...
        if (request.header(str2) == null) {
            newBuilder.header(str2, HTTP.CONN_KEEP_ALIVE);
        }
        String str3 = "Accept-Encoding";
        if (request.header(str3) == null) {
            this.transparentGzip = true;
            newBuilder.header(str3, HttpRequest.ENCODING_GZIP);
        }
    ...
    private static boolean validate(Response response, Response response2) {
        ...
        Date date = response.headers().getDate(str);
        if (date != null) {
    ...
    private static Headers combine(Headers headers, Headers headers2) throws IOException {
        ...
        for (int i = 0; i < size; i++) {
            ...
            if ((!"Warning".equalsIgnoreCase(name) || !value.startsWith(IcyHeaders.REQUEST_HEADER_ENABLE_METADATA_VALUE)) && (!OkHeaders.isEndToEnd(name) || headers2.get(name) == null)) {
