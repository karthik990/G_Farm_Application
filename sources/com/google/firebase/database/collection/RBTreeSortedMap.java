...
public class RBTreeSortedMap<K, V> extends ImmutableSortedMap<K, V> {
    ...
    private static class Builder<A, B, C> {
        ...
        private Builder(List<A> list, Map<B, C> map, KeyTranslator<A, B> keyTranslator2) {
            this.keys = list;
            this.values = map;
        ...
        private void buildPennant(Color color, int i, int i2) {
            ...
            if (color == Color.RED) {
                lLRBValueNode = new LLRBRedValueNode<>(obj, getValue(obj), null, buildBalancedTree);
            } else {
                lLRBValueNode = new LLRBBlackValueNode<>(obj, getValue(obj), null, buildBalancedTree);
            }
            if (this.root == null) {
        ...
        public static <A, B, C> RBTreeSortedMap<A, C> buildFrom(List<A> list, Map<B, C> map, KeyTranslator<A, B> keyTranslator2, Comparator<A> comparator) {
    ...
    public static <A, B, C> RBTreeSortedMap<A, C> buildFrom(List<A> list, Map<B, C> map, KeyTranslator<A, B> keyTranslator, Comparator<A> comparator2) {
    ...
    public static <A, B> RBTreeSortedMap<A, B> fromMap(Map<A, B> map, Comparator<A> comparator2) {
        return Builder.buildFrom(new ArrayList(map.keySet()), map, com.google.firebase.database.collection.ImmutableSortedMap.Builder.identityTranslator(), comparator2);
    }
}
