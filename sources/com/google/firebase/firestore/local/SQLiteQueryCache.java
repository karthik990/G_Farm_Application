...
final class SQLiteQueryCache implements QueryCache {
    ...
    /* renamed from: db */
    private final SQLitePersistence f2032db;
    ...
    SQLiteQueryCache(SQLitePersistence sQLitePersistence, LocalSerializer localSerializer2) {
        this.f2032db = sQLitePersistence;
    ...
    public void start() {
        boolean z = true;
        if (this.f2032db.query("SELECT highest_target_id, highest_listen_sequence_number, last_remote_snapshot_version_seconds, last_remote_snapshot_version_nanos, target_count FROM target_globals LIMIT 1").first(SQLiteQueryCache$$Lambda$1.lambdaFactory$(this)) != 1) {
            z = false;
        }
    ...
    public void forEachTarget(Consumer<QueryData> consumer) {
        this.f2032db.query("SELECT target_proto FROM targets").forEach(SQLiteQueryCache$$Lambda$2.lambdaFactory$(this, consumer));
    }
    ...
    public int removeQueries(long j, SparseArray<?> sparseArray) {
        int[] iArr = new int[1];
        this.f2032db.query("SELECT target_id FROM targets WHERE last_listen_sequence_number <= ?").binding(Long.valueOf(j)).forEach(SQLiteQueryCache$$Lambda$3.lambdaFactory$(this, sparseArray, iArr));
        writeMetadata();
        return iArr[0];
    }
    ...
    public QueryData getQueryData(Query query) {
        ...
        this.f2032db.query("SELECT target_proto FROM targets WHERE canonical_id = ?").binding(canonicalId).forEach(SQLiteQueryCache$$Lambda$4.lambdaFactory$(this, query, queryDataHolder));
    ...
    public void addMatchingKeys(ImmutableSortedSet<DocumentKey> immutableSortedSet, int i) {
        SQLiteStatement prepare = this.f2032db.prepare("INSERT OR IGNORE INTO target_documents (target_id, path) VALUES (?, ?)");
        SQLiteLruReferenceDelegate referenceDelegate = this.f2032db.getReferenceDelegate();
        Iterator it = immutableSortedSet.iterator();
        while (it.hasNext()) {
            DocumentKey documentKey = (DocumentKey) it.next();
            String encode = EncodedPath.encode(documentKey.getPath());
            this.f2032db.execute(prepare, Integer.valueOf(i), encode);
    ...
    public void removeMatchingKeys(ImmutableSortedSet<DocumentKey> immutableSortedSet, int i) {
        SQLiteStatement prepare = this.f2032db.prepare("DELETE FROM target_documents WHERE target_id = ? AND path = ?");
        SQLiteLruReferenceDelegate referenceDelegate = this.f2032db.getReferenceDelegate();
        Iterator it = immutableSortedSet.iterator();
        while (it.hasNext()) {
            DocumentKey documentKey = (DocumentKey) it.next();
    ...
    private void removeMatchingKeysForTargetId(int i) {
        this.f2032db.execute("DELETE FROM target_documents WHERE target_id = ?", Integer.valueOf(i));
    }
    ...
    public ImmutableSortedSet<DocumentKey> getMatchingKeysForTargetId(int i) {
        ...
        this.f2032db.query("SELECT path FROM target_documents WHERE target_id = ?").binding(Integer.valueOf(i)).forEach(SQLiteQueryCache$$Lambda$5.lambdaFactory$(documentKeysHolder));
        return documentKeysHolder.keys;
    }

    public boolean containsKey(DocumentKey documentKey) {
        String encode = EncodedPath.encode(documentKey.getPath());
        return !this.f2032db.query("SELECT target_id FROM target_documents WHERE path = ? AND target_id != 0 LIMIT 1").binding(encode).isEmpty();
    }
}
