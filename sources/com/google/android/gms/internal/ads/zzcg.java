package com.google.android.gms.internal.ads;

import android.app.Activity;
import android.content.Context;
import android.support.p009v4.media.session.PlaybackStateCompat;
import android.text.TextUtils;
import android.util.DisplayMetrics;
import android.util.Log;
import android.view.MotionEvent;
import android.view.View;
import java.lang.reflect.InvocationTargetException;
import java.lang.reflect.Method;
import java.util.ArrayList;
import java.util.Arrays;
import java.util.List;
import java.util.concurrent.Callable;
import java.util.concurrent.ExecutorService;
import java.util.concurrent.TimeUnit;

public class zzcg extends zzcf {
    private static final String TAG = zzcg.class.getSimpleName();
    private static long startTime = 0;
    private static final Object zzqr = new Object();
    private static boolean zzqs = false;
    protected boolean zzqt = false;
    private String zzqu;
    private boolean zzqv = false;
    private boolean zzqw = false;
    private zzdi zzqx;

    protected zzcg(Context context, String str, boolean z) {
        super(context);
        this.zzqu = str;
        this.zzqt = z;
    }

    private static zzdf zza(zzcz zzcz, MotionEvent motionEvent, DisplayMetrics displayMetrics) throws zzcw {
        Method zza = zzcz.zza("ryulhTIO44CH2QVINEPOtPkpQOSdbbq7HgisLeHH33VCs+TjwYWzYttvzJcQiI1k", "dnKLchXhJgI3U7L4th0PQJWe3DOIJJ7MUji5p7t5mjc=");
        if (zza == null || motionEvent == null) {
            throw new zzcw();
        }
        try {
            return new zzdf((String) zza.invoke(null, new Object[]{motionEvent, displayMetrics}));
        } catch (IllegalAccessException | InvocationTargetException e) {
            throw new zzcw(e);
        }
    }

    protected static synchronized void zza(Context context, boolean z) {
        synchronized (zzcg.class) {
            if (!zzqs) {
                startTime = System.currentTimeMillis() / 1000;
                zzps = zzb(context, z);
                zzqs = true;
            }
        }
    }

    private static void zza(List<Callable<Void>> list) {
        if (zzps != null) {
            ExecutorService zzab = zzps.zzab();
            if (zzab != null && !list.isEmpty()) {
                try {
                    zzab.invokeAll(list, ((Long) zzkb.zzik().zzd(zznk.zzbaq)).longValue(), TimeUnit.MILLISECONDS);
                } catch (InterruptedException e) {
                    Log.d(TAG, String.format("class methods got exception: %s", new Object[]{zzdg.zza((Throwable) e)}));
                }
            }
        }
    }

    private static zzcz zzb(Context context, boolean z) {
        if (zzps == null) {
            synchronized (zzqr) {
                if (zzps == null) {
                    zzcz zza = zzcz.zza(context, "fFhi0cTZpyVQYwMwl7BCfa0fa6esmkRUaNr4ktfJYZ8=", "", z);
                    if (zza.isInitialized()) {
                        if (((Boolean) zzkb.zzik().zzd(zznk.zzbaw)).booleanValue()) {
                            zza.zza("4o7tecxtkw7XaNt5hPj+0H1LvOi0SgxCIJTY9VcbazM/HSl/sFlxBFwnc8glnvoB", "RgSY6YxU2k1vLXOV3vapBnQwJDzYDlmX50wbm2tDcnw=", new Class[0]);
                        }
                        zza.zza("1QeH3Cf7T53ayw17Ebbo9YTdhU+IFx0X5nCtC5gZQym4uicOVPXxYWmMK9k58i8n", "bHJRpFJ+2R5LAbYQUBDMyfYpLd1oiGixlpIqMJOBQPY=", Context.class);
                        zza.zza("1DzTSPdcTVkbB0cTQEaPo/MU1E1TGypt38nYJILt/8CcQbOuYjF6hLShUHnLb5uw", "dkCo8xze5sFyp7Msrb8F1JKjVIP7LutspgZPYQvsMlo=", Context.class);
                        zza.zza("+8WN7RfVBrTKK5NXM+y9JA4Ygu2jPZokHNFGgulhBRZ32uCnyKWWGQNTE1weoQCM", "peILYfFejTn8xmazfpiZWeK2eaYjjxEBmAZ8bUD9PWU=", Context.class);
                        zza.zza("6hrke/fCJ5tLCck7L6DvYxDB/To2PLUVGD3lbM1fdL2y9IScTypVRvrPFLLn4rDM", "+GAY853+DQRMYkeMG8Hbv1THvRgwf43Jh+3p4yQDqGw=", Context.class);
                        zza.zza("f8J+j+UhiTEjRF3iyDcFnM9Rm6LC/P+2PE2qff5S0gHSDPzrH4HNcnCaI+twvhbv", "MOBosqV6pejtL/WEgsHP3Y2p2WtApcEEa2XyqqRQCo0=", Context.class);
                        zza.zza("bYi99wWoqjOnUF4nnSWQsu9qLqARRsoauNez1uhJPws2YAlolLPh/xgKDnyI06MJ", "pjeIE8EsMfMk4SIebXHXNWKXTpK8v5vG6YqOxp1QH1M=", Context.class);
                        zza.zza("ug+duzFNTlMWhYez8qKOR3bxkM8vLOPy9xVUBCCMTrplDFf/BnF2bNmA7hw0iYtI", "ogMtakENM6NAXJL7DCVTIbgawd5T3ZVhEaXugMAz+D4=", Context.class);
                        zza.zza("PRnAd1qZLf3/u2x/ZslcDq2vQzDbmFHkH/jSij536nFGwYo+g6o8PpQOJcDeuz++", "99eM4BYfXxOCBAhS2wQJENWQ4BMWrR/wKqrfHx1Ws8w=", Context.class);
                        zza.zza("ryulhTIO44CH2QVINEPOtPkpQOSdbbq7HgisLeHH33VCs+TjwYWzYttvzJcQiI1k", "dnKLchXhJgI3U7L4th0PQJWe3DOIJJ7MUji5p7t5mjc=", MotionEvent.class, DisplayMetrics.class);
                        zza.zza("oT49Nu6m3oNdAQwbQzvwr/RE4TwZbR8mJKypakrI/GqBgyD2niyVic8N9UaqvU10", "LS3Cw7DlWSqpRsXrajoVsCxXOedCBnQXFZcLjXzD8so=", MotionEvent.class, DisplayMetrics.class);
                        zza.zza("1GzFbNgtZBsZ6xBrCbOvmw4GuyabWdtXgJqf+Aoik3E00oSTfDga4UiXvbD1YLYB", "1EiWqNFog84woyMiTveFonfDO5rsRz44QrKedC4xw+8=", new Class[0]);
                        zza.zza("R1L5/7J00Fko/kDi6VDsBhCzpp4/7wzabnzdihUS7rPLvaXLFWrERChyvYYgVJRF", "4WtEOgM4wQ7wiw1OB3pMyAbaplBizcjE6LRBT/nrDgM=", new Class[0]);
                        zza.zza("uFEr0moMuWuurgrUpfSBjyUksFv6IqKMSuNqo2jCjRmu9JHDaSBf9LDKs2u/XE/B", "hDtufeJTxJSs5qQMCIPuyzc9ZPOtO/34yyf8Kf44rGo=", new Class[0]);
                        zza.zza("C4t4eP72HKoZsX62gBXGicLgvQbyKikYhXkvESjpMVAOEyUibmrLmpkJ5lZygzZE", "onbJNRechKgg7ilJYzLDEQ07M0uNlDZp05nZ4h6WJ40=", new Class[0]);
                        zza.zza("YOE4tLd8IhAbsNGoUEFHfu51/79yG0YlK4bMxi/e6gOIrtpqPN+TQgqpfYNBoiRy", "aFY444Zz3/sMYl+Q7uujofvlLWJ37651UMbaKJCFBnU=", new Class[0]);
                        zza.zza("Q7gHsedaZvBKvqRLQGMIPKihViSMmXus6bBgU0lNdv+Pm9sSlP+Z+JltWG56lF1K", "mrdNgIRzsAu+cEIamFRkeCivuYIijUjvD76p38JyQec=", new Class[0]);
                        zza.zza("LJD7GILw6ox/o3kYqhR5FF4FDylErrAaJ66Y8V8gtnNGF8DcetGJrYvTlHeK8kyR", "GKdCM/OywOIR4FMFS5XrDdTIpVvhoMFma/KSfRUdsbQ=", Context.class, Boolean.TYPE, Boolean.TYPE);
                        zza.zza("nXKe8Ev09tosW935mj67BeJvnsHiQzgrotB4vyuZm+aJQgx77SXcf757aVGtnDOm", "yl+F7AaTCLci2yEfPOMTGPVVFLoqw9fKWSf4HTpN46M=", StackTraceElement[].class);
                        zza.zza("j5/WVRJsgiWNVIg0a6aXYQYocHm4rEEozf0LrIkPezct93ISwgyVBTLRIkEvztpn", "gCYklRkrI0pyKJ3j1u5Yl6Jj12a6PD1K39diuw7+f7o=", View.class, DisplayMetrics.class);
                        zza.zza("WiMYovSpRj6RF1r6zXON0hYVyLxgllUAYyMyyeBevdbJbdYcoBpd8nDjCwwclcSu", "vpq7z91A+vN3MIS65A+GwiFRgEFOFpQOdK5Z+mjW21w=", Context.class, Boolean.TYPE);
                        zza.zza("wxabFp8V9bGKut15duzoi2M8ew0t/HWGFw3NoPp8pFanKwQpdIx0WzpFbmJaLX9T", "grSrEy+Ml8J7boJNrN9uNiXpddsqwR6A9NVesV/hHT8=", View.class, Activity.class, Boolean.TYPE);
                        zza.zza("aH422z6bUtRL57gOABo0AU5IQDHmWvjsQgQZk64bzhjXVYUZmajAsUz/rrnMYVsw", "UAmRxgiWCEqHprf42V+e65VvIvUfFXeXbaMy5B8vzwA=", Long.TYPE);
                    }
                    zzps = zza;
                }
            }
        }
        return zzps;
    }

    /* access modifiers changed from: protected */
    public final long zza(StackTraceElement[] stackTraceElementArr) throws zzcw {
        Method zza = zzps.zza("nXKe8Ev09tosW935mj67BeJvnsHiQzgrotB4vyuZm+aJQgx77SXcf757aVGtnDOm", "yl+F7AaTCLci2yEfPOMTGPVVFLoqw9fKWSf4HTpN46M=");
        if (zza == null || stackTraceElementArr == null) {
            throw new zzcw();
        }
        try {
            return new zzcx((String) zza.invoke(null, new Object[]{stackTraceElementArr})).zzro.longValue();
        } catch (IllegalAccessException | InvocationTargetException e) {
            throw new zzcw(e);
        }
    }

    /* access modifiers changed from: protected */
    public final zzba zza(Context context, View view, Activity activity) {
        List list;
        Long l;
        zzba zzba = new zzba();
        if (!TextUtils.isEmpty(this.zzqu)) {
            zzba.zzcw = this.zzqu;
        }
        zzcz zzb = zzb(context, this.zzqt);
        if (!zzb.isInitialized()) {
            zzba.zzdu = Long.valueOf(PlaybackStateCompat.ACTION_PREPARE);
            list = Arrays.asList(new Callable[]{new zzdt(zzb, zzba)});
        } else {
            try {
                zzdf zza = zza(zzb, this.zzpy, this.zzqq);
                zzba.zzdo = zza.zzsp;
                zzba.zzdp = zza.zzsq;
                zzba.zzdq = zza.zzsr;
                if (this.zzqp) {
                    zzba.zzec = zza.zzfr;
                    zzba.zzed = zza.zzfp;
                }
            } catch (zzcw unused) {
            }
            zzbb zzbb = new zzbb();
            if (this.zzqa > 0 && zzdg.zza(this.zzqq)) {
                zzbb.zzfz = Long.valueOf(zzdg.zza(this.zzqh, this.zzqq));
                zzbb.zzga = Long.valueOf(zzdg.zza((double) (this.zzqm - this.zzqk), this.zzqq));
                zzbb.zzgb = Long.valueOf(zzdg.zza((double) (this.zzqn - this.zzql), this.zzqq));
                zzbb.zzge = Long.valueOf(zzdg.zza((double) this.zzqk, this.zzqq));
                zzbb.zzgf = Long.valueOf(zzdg.zza((double) this.zzql, this.zzqq));
                if (this.zzqp && this.zzpy != null) {
                    long zza2 = zzdg.zza((double) (((this.zzqk - this.zzqm) + this.zzpy.getRawX()) - this.zzpy.getX()), this.zzqq);
                    if (zza2 != 0) {
                        zzbb.zzgc = Long.valueOf(zza2);
                    }
                    long zza3 = zzdg.zza((double) (((this.zzql - this.zzqn) + this.zzpy.getRawY()) - this.zzpy.getY()), this.zzqq);
                    if (zza3 != 0) {
                        zzbb.zzgd = Long.valueOf(zza3);
                    }
                }
            }
            try {
                zzdf zzb2 = zzb(this.zzpy);
                zzbb.zzdo = zzb2.zzsp;
                zzbb.zzdp = zzb2.zzsq;
                zzbb.zzfu = zzb2.zzsr;
                if (this.zzqp) {
                    zzbb.zzfp = zzb2.zzfp;
                    zzbb.zzfr = zzb2.zzfr;
                    zzbb.zzft = Integer.valueOf(zzb2.zzss.longValue() != 0 ? 1 : 0);
                    if (this.zzqb > 0) {
                        if (zzdg.zza(this.zzqq)) {
                            double d = (double) this.zzqg;
                            double d2 = (double) this.zzqb;
                            Double.isNaN(d);
                            Double.isNaN(d2);
                            l = Long.valueOf(Math.round(d / d2));
                        } else {
                            l = null;
                        }
                        zzbb.zzfq = l;
                        double d3 = (double) this.zzqf;
                        double d4 = (double) this.zzqb;
                        Double.isNaN(d3);
                        Double.isNaN(d4);
                        zzbb.zzfs = Long.valueOf(Math.round(d3 / d4));
                    }
                    zzbb.zzfw = zzb2.zzfw;
                    zzbb.zzfv = zzb2.zzfv;
                    zzbb.zzfx = Integer.valueOf(zzb2.zzsv.longValue() != 0 ? 1 : 0);
                }
            } catch (zzcw unused2) {
            }
            if (this.zzqe > 0) {
                zzbb.zzfy = Long.valueOf(this.zzqe);
            }
            zzba.zzet = zzbb;
            if (this.zzqa > 0) {
                zzba.zzeh = Long.valueOf(this.zzqa);
            }
            if (this.zzqb > 0) {
                zzba.zzeg = Long.valueOf(this.zzqb);
            }
            if (this.zzqc > 0) {
                zzba.zzef = Long.valueOf(this.zzqc);
            }
            if (this.zzqd > 0) {
                zzba.zzei = Long.valueOf(this.zzqd);
            }
            try {
                int size = this.zzpz.size() - 1;
                if (size > 0) {
                    zzba.zzeu = new zzbb[size];
                    for (int i = 0; i < size; i++) {
                        zzdf zza4 = zza(zzps, (MotionEvent) this.zzpz.get(i), this.zzqq);
                        zzbb zzbb2 = new zzbb();
                        zzbb2.zzdo = zza4.zzsp;
                        zzbb2.zzdp = zza4.zzsq;
                        zzba.zzeu[i] = zzbb2;
                    }
                }
            } catch (zzcw unused3) {
                zzba.zzeu = null;
            }
            list = new ArrayList();
            if (zzb.zzab() != null) {
                int zzx = zzb.zzx();
                list.add(new zzdt(zzb, zzba));
                zzcz zzcz = zzb;
                zzba zzba2 = zzba;
                zzdw zzdw = new zzdw(zzcz, "uFEr0moMuWuurgrUpfSBjyUksFv6IqKMSuNqo2jCjRmu9JHDaSBf9LDKs2u/XE/B", "hDtufeJTxJSs5qQMCIPuyzc9ZPOtO/34yyf8Kf44rGo=", zzba2, zzx, 1);
                list.add(zzdw);
                zzdr zzdr = new zzdr(zzcz, "1GzFbNgtZBsZ6xBrCbOvmw4GuyabWdtXgJqf+Aoik3E00oSTfDga4UiXvbD1YLYB", "1EiWqNFog84woyMiTveFonfDO5rsRz44QrKedC4xw+8=", zzba2, startTime, zzx, 25);
                list.add(zzdr);
                int i2 = zzx;
                zzdq zzdq = new zzdq(zzcz, "C4t4eP72HKoZsX62gBXGicLgvQbyKikYhXkvESjpMVAOEyUibmrLmpkJ5lZygzZE", "onbJNRechKgg7ilJYzLDEQ07M0uNlDZp05nZ4h6WJ40=", zzba2, i2, 44);
                list.add(zzdq);
                zzdv zzdv = new zzdv(zzcz, "f8J+j+UhiTEjRF3iyDcFnM9Rm6LC/P+2PE2qff5S0gHSDPzrH4HNcnCaI+twvhbv", "MOBosqV6pejtL/WEgsHP3Y2p2WtApcEEa2XyqqRQCo0=", zzba2, i2, 12);
                list.add(zzdv);
                zzdx zzdx = new zzdx(zzcz, "bYi99wWoqjOnUF4nnSWQsu9qLqARRsoauNez1uhJPws2YAlolLPh/xgKDnyI06MJ", "pjeIE8EsMfMk4SIebXHXNWKXTpK8v5vG6YqOxp1QH1M=", zzba2, i2, 3);
                list.add(zzdx);
                zzdu zzdu = new zzdu(zzcz, "YOE4tLd8IhAbsNGoUEFHfu51/79yG0YlK4bMxi/e6gOIrtpqPN+TQgqpfYNBoiRy", "aFY444Zz3/sMYl+Q7uujofvlLWJ37651UMbaKJCFBnU=", zzba2, i2, 22);
                list.add(zzdu);
                zzdp zzdp = new zzdp(zzcz, "6hrke/fCJ5tLCck7L6DvYxDB/To2PLUVGD3lbM1fdL2y9IScTypVRvrPFLLn4rDM", "+GAY853+DQRMYkeMG8Hbv1THvRgwf43Jh+3p4yQDqGw=", zzba2, i2, 5);
                list.add(zzdp);
                zzee zzee = new zzee(zzcz, "ug+duzFNTlMWhYez8qKOR3bxkM8vLOPy9xVUBCCMTrplDFf/BnF2bNmA7hw0iYtI", "ogMtakENM6NAXJL7DCVTIbgawd5T3ZVhEaXugMAz+D4=", zzba2, i2, 48);
                list.add(zzee);
                zzdm zzdm = new zzdm(zzcz, "PRnAd1qZLf3/u2x/ZslcDq2vQzDbmFHkH/jSij536nFGwYo+g6o8PpQOJcDeuz++", "99eM4BYfXxOCBAhS2wQJENWQ4BMWrR/wKqrfHx1Ws8w=", zzba2, i2, 49);
                list.add(zzdm);
                zzeb zzeb = new zzeb(zzcz, "Q7gHsedaZvBKvqRLQGMIPKihViSMmXus6bBgU0lNdv+Pm9sSlP+Z+JltWG56lF1K", "mrdNgIRzsAu+cEIamFRkeCivuYIijUjvD76p38JyQec=", zzba2, i2, 51);
                list.add(zzeb);
                zzea zzea = new zzea(zzcz, "nXKe8Ev09tosW935mj67BeJvnsHiQzgrotB4vyuZm+aJQgx77SXcf757aVGtnDOm", "yl+F7AaTCLci2yEfPOMTGPVVFLoqw9fKWSf4HTpN46M=", zzba2, i2, 45, new Throwable().getStackTrace());
                list.add(zzea);
                zzef zzef = new zzef(zzcz, "j5/WVRJsgiWNVIg0a6aXYQYocHm4rEEozf0LrIkPezct93ISwgyVBTLRIkEvztpn", "gCYklRkrI0pyKJ3j1u5Yl6Jj12a6PD1K39diuw7+f7o=", zzba2, i2, 57, view);
                list.add(zzef);
                zzdz zzdz = new zzdz(zzcz, "WiMYovSpRj6RF1r6zXON0hYVyLxgllUAYyMyyeBevdbJbdYcoBpd8nDjCwwclcSu", "vpq7z91A+vN3MIS65A+GwiFRgEFOFpQOdK5Z+mjW21w=", zzba2, i2, 61);
                list.add(zzdz);
                if (((Boolean) zzkb.zzik().zzd(zznk.zzbar)).booleanValue()) {
                    zzdl zzdl = new zzdl(zzb, "wxabFp8V9bGKut15duzoi2M8ew0t/HWGFw3NoPp8pFanKwQpdIx0WzpFbmJaLX9T", "grSrEy+Ml8J7boJNrN9uNiXpddsqwR6A9NVesV/hHT8=", zzba, zzx, 62, view, activity);
                    list.add(zzdl);
                }
                if (((Boolean) zzkb.zzik().zzd(zznk.zzbat)).booleanValue()) {
                    zzec zzec = new zzec(zzb, "aH422z6bUtRL57gOABo0AU5IQDHmWvjsQgQZk64bzhjXVYUZmajAsUz/rrnMYVsw", "UAmRxgiWCEqHprf42V+e65VvIvUfFXeXbaMy5B8vzwA=", zzba, zzx, 53, this.zzqx);
                    list.add(zzec);
                }
            }
        }
        zza(list);
        return zzba;
    }

    /* access modifiers changed from: protected */
    public final zzba zza(Context context, zzax zzax) {
        zzba zzba = new zzba();
        if (!TextUtils.isEmpty(this.zzqu)) {
            zzba.zzcw = this.zzqu;
        }
        zzcz zzb = zzb(context, this.zzqt);
        if (zzb.zzab() != null) {
            zza(zza(zzb, zzba, (zzax) null));
        }
        return zzba;
    }

    /* access modifiers changed from: protected */
    public List<Callable<Void>> zza(zzcz zzcz, zzba zzba, zzax zzax) {
        int zzx = zzcz.zzx();
        ArrayList arrayList = new ArrayList();
        if (!zzcz.isInitialized()) {
            zzba.zzdu = Long.valueOf(PlaybackStateCompat.ACTION_PREPARE);
            return arrayList;
        }
        zzcz zzcz2 = zzcz;
        zzba zzba2 = zzba;
        zzdo zzdo = new zzdo(zzcz2, "LJD7GILw6ox/o3kYqhR5FF4FDylErrAaJ66Y8V8gtnNGF8DcetGJrYvTlHeK8kyR", "GKdCM/OywOIR4FMFS5XrDdTIpVvhoMFma/KSfRUdsbQ=", zzba2, zzx, 27, zzax);
        arrayList.add(zzdo);
        zzdr zzdr = new zzdr(zzcz2, "1GzFbNgtZBsZ6xBrCbOvmw4GuyabWdtXgJqf+Aoik3E00oSTfDga4UiXvbD1YLYB", "1EiWqNFog84woyMiTveFonfDO5rsRz44QrKedC4xw+8=", zzba2, startTime, zzx, 25);
        arrayList.add(zzdr);
        int i = zzx;
        zzdw zzdw = new zzdw(zzcz2, "uFEr0moMuWuurgrUpfSBjyUksFv6IqKMSuNqo2jCjRmu9JHDaSBf9LDKs2u/XE/B", "hDtufeJTxJSs5qQMCIPuyzc9ZPOtO/34yyf8Kf44rGo=", zzba2, i, 1);
        arrayList.add(zzdw);
        zzdy zzdy = new zzdy(zzcz2, "+8WN7RfVBrTKK5NXM+y9JA4Ygu2jPZokHNFGgulhBRZ32uCnyKWWGQNTE1weoQCM", "peILYfFejTn8xmazfpiZWeK2eaYjjxEBmAZ8bUD9PWU=", zzba2, i, 31);
        arrayList.add(zzdy);
        zzed zzed = new zzed(zzcz2, "R1L5/7J00Fko/kDi6VDsBhCzpp4/7wzabnzdihUS7rPLvaXLFWrERChyvYYgVJRF", "4WtEOgM4wQ7wiw1OB3pMyAbaplBizcjE6LRBT/nrDgM=", zzba2, i, 33);
        arrayList.add(zzed);
        zzdn zzdn = new zzdn(zzcz2, "1DzTSPdcTVkbB0cTQEaPo/MU1E1TGypt38nYJILt/8CcQbOuYjF6hLShUHnLb5uw", "dkCo8xze5sFyp7Msrb8F1JKjVIP7LutspgZPYQvsMlo=", zzba2, i, 29);
        arrayList.add(zzdn);
        zzdp zzdp = new zzdp(zzcz2, "6hrke/fCJ5tLCck7L6DvYxDB/To2PLUVGD3lbM1fdL2y9IScTypVRvrPFLLn4rDM", "+GAY853+DQRMYkeMG8Hbv1THvRgwf43Jh+3p4yQDqGw=", zzba2, i, 5);
        arrayList.add(zzdp);
        zzdv zzdv = new zzdv(zzcz2, "f8J+j+UhiTEjRF3iyDcFnM9Rm6LC/P+2PE2qff5S0gHSDPzrH4HNcnCaI+twvhbv", "MOBosqV6pejtL/WEgsHP3Y2p2WtApcEEa2XyqqRQCo0=", zzba2, i, 12);
        arrayList.add(zzdv);
        zzdx zzdx = new zzdx(zzcz2, "bYi99wWoqjOnUF4nnSWQsu9qLqARRsoauNez1uhJPws2YAlolLPh/xgKDnyI06MJ", "pjeIE8EsMfMk4SIebXHXNWKXTpK8v5vG6YqOxp1QH1M=", zzba2, i, 3);
        arrayList.add(zzdx);
        zzdq zzdq = new zzdq(zzcz2, "C4t4eP72HKoZsX62gBXGicLgvQbyKikYhXkvESjpMVAOEyUibmrLmpkJ5lZygzZE", "onbJNRechKgg7ilJYzLDEQ07M0uNlDZp05nZ4h6WJ40=", zzba2, i, 44);
        arrayList.add(zzdq);
        zzdu zzdu = new zzdu(zzcz2, "YOE4tLd8IhAbsNGoUEFHfu51/79yG0YlK4bMxi/e6gOIrtpqPN+TQgqpfYNBoiRy", "aFY444Zz3/sMYl+Q7uujofvlLWJ37651UMbaKJCFBnU=", zzba2, i, 22);
        arrayList.add(zzdu);
        zzee zzee = new zzee(zzcz2, "ug+duzFNTlMWhYez8qKOR3bxkM8vLOPy9xVUBCCMTrplDFf/BnF2bNmA7hw0iYtI", "ogMtakENM6NAXJL7DCVTIbgawd5T3ZVhEaXugMAz+D4=", zzba2, i, 48);
        arrayList.add(zzee);
        zzdm zzdm = new zzdm(zzcz2, "PRnAd1qZLf3/u2x/ZslcDq2vQzDbmFHkH/jSij536nFGwYo+g6o8PpQOJcDeuz++", "99eM4BYfXxOCBAhS2wQJENWQ4BMWrR/wKqrfHx1Ws8w=", zzba2, i, 49);
        arrayList.add(zzdm);
        zzeb zzeb = new zzeb(zzcz2, "Q7gHsedaZvBKvqRLQGMIPKihViSMmXus6bBgU0lNdv+Pm9sSlP+Z+JltWG56lF1K", "mrdNgIRzsAu+cEIamFRkeCivuYIijUjvD76p38JyQec=", zzba2, i, 51);
        arrayList.add(zzeb);
        zzdz zzdz = new zzdz(zzcz2, "WiMYovSpRj6RF1r6zXON0hYVyLxgllUAYyMyyeBevdbJbdYcoBpd8nDjCwwclcSu", "vpq7z91A+vN3MIS65A+GwiFRgEFOFpQOdK5Z+mjW21w=", zzba2, i, 61);
        arrayList.add(zzdz);
        return arrayList;
    }

    /* access modifiers changed from: protected */
    public final zzdf zzb(MotionEvent motionEvent) throws zzcw {
        Method zza = zzps.zza("oT49Nu6m3oNdAQwbQzvwr/RE4TwZbR8mJKypakrI/GqBgyD2niyVic8N9UaqvU10", "LS3Cw7DlWSqpRsXrajoVsCxXOedCBnQXFZcLjXzD8so=");
        if (zza == null || motionEvent == null) {
            throw new zzcw();
        }
        try {
            return new zzdf((String) zza.invoke(null, new Object[]{motionEvent, this.zzqq}));
        } catch (IllegalAccessException | InvocationTargetException e) {
            throw new zzcw(e);
        }
    }

    public final void zzb(View view) {
        if (((Boolean) zzkb.zzik().zzd(zznk.zzbat)).booleanValue()) {
            zzdi zzdi = this.zzqx;
            if (zzdi == null) {
                this.zzqx = new zzdi(zzps, view);
            } else {
                zzdi.zzd(view);
            }
        }
    }
}
