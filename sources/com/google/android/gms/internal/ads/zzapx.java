...
public final class zzapx {
    ...
    public final void onStop() {
        if (((Boolean) zzkb.zzik().zzd(zznk.zzavd)).booleanValue() && !this.zzdam) {
            ...
            bundle.putString(VineCardUtils.PLAYER_CARD, this.zzdal.zzsp());
            for (zzalr zzalr : this.zzdae.zzry()) {
                String str = "fps_c_";
                String valueOf = String.valueOf(zzalr.name);
                bundle.putString(valueOf.length() != 0 ? str.concat(valueOf) : new String(str), Integer.toString(zzalr.count));
                String str2 = "fps_p_";
                String valueOf2 = String.valueOf(zzalr.name);
                bundle.putString(valueOf2.length() != 0 ? str2.concat(valueOf2) : new String(str2), Double.toString(zzalr.zzctb));
            }
            int i = 0;
            while (true) {
                long[] jArr = this.zzdaf;
                if (i < jArr.length) {
                    String str3 = this.zzdag[i];
                    if (str3 != null) {
    ...
    public final void zzc(zzapg zzapg) {
        if (this.zzdaj && !this.zzdak) {
            if (zzakb.zzqp() && !this.zzdak) {
                zzakb.m1419v("VideoMetricsMixin first frame");
            }
