...
public final class CollapsingTextHelper {
    ...
    private void drawMultinlineTransition(Canvas canvas, float f, float f2, float f3, float f4) {
        ...
        canvas.drawText(str, 0, Math.min(this.textLayout.getLineEnd(0), str.length()), 0.0f, f6 / this.scale, this.textPaint);
    }
    ...
    private StaticLayout createStaticLayout(int i, float f, boolean z) {
        ...
        try {
            staticLayout = StaticLayoutBuilderCompat.obtain(this.text, this.textPaint, (int) f).setEllipsize(TruncateAt.END).setIsRtl(z).setAlignment(Alignment.ALIGN_NORMAL).setIncludePad(false).setMaxLines(i).build();
