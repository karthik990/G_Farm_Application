...
public class TooltipDrawable extends MaterialShapeDrawable implements TextDrawableDelegate {
    private static final int DEFAULT_STYLE = C2556R.C2562style.Widget_MaterialComponents_Tooltip;
    private static final int DEFAULT_THEME_ATTR = C2556R.attr.tooltipStyle;
    ...
    private void loadFromAttributes(AttributeSet attributeSet, int i, int i2) {
        ...
        TypedArray obtainStyledAttributes = ThemeEnforcement.obtainStyledAttributes(this.context, attributeSet, C2556R.styleable.Tooltip, i, i2, new int[0]);
        this.arrowSize = this.context.getResources().getDimensionPixelSize(C2556R.dimen.mtrl_tooltip_arrowSize);
        setShapeAppearanceModel(getShapeAppearanceModel().toBuilder().setBottomEdge(createMarkerEdge()).build());
        setText(obtainStyledAttributes.getText(C2556R.styleable.Tooltip_android_text));
        setTextAppearance(MaterialResources.getTextAppearance(this.context, obtainStyledAttributes, C2556R.styleable.Tooltip_android_textAppearance));
        setFillColor(ColorStateList.valueOf(obtainStyledAttributes.getColor(C2556R.styleable.Tooltip_backgroundTint, MaterialColors.layer(ColorUtils.setAlphaComponent(MaterialColors.getColor(this.context, 16842801, cls.getCanonicalName()), 229), ColorUtils.setAlphaComponent(MaterialColors.getColor(this.context, C2556R.attr.colorOnBackground, cls.getCanonicalName()), Opcodes.IFEQ)))));
        setStrokeColor(ColorStateList.valueOf(MaterialColors.getColor(this.context, C2556R.attr.colorSurface, cls.getCanonicalName())));
        this.padding = obtainStyledAttributes.getDimensionPixelSize(C2556R.styleable.Tooltip_android_padding, 0);
        this.minWidth = obtainStyledAttributes.getDimensionPixelSize(C2556R.styleable.Tooltip_android_minWidth, 0);
        this.minHeight = obtainStyledAttributes.getDimensionPixelSize(C2556R.styleable.Tooltip_android_minHeight, 0);
        this.layoutMargin = obtainStyledAttributes.getDimensionPixelSize(C2556R.styleable.Tooltip_android_layout_margin, 0);
    ...
    private EdgeTreatment createMarkerEdge() {
        ...
        return new OffsetEdgeTreatment(new MarkerEdgeTreatment((float) this.arrowSize), Math.min(Math.max(f, -f2), f2));
    }
