...
public class ShapeAppearancePathProvider {
    ...
    public void calculatePath(ShapeAppearanceModel shapeAppearanceModel, float f, RectF rectF, PathListener pathListener, Path path) {
        ...
        this.boundsPath.addRect(rectF, Direction.CW);
        ...
        if (VERSION.SDK_INT >= 19 && !this.overlappedEdgePath.isEmpty()) {
            path.op(this.overlappedEdgePath, Op.UNION);
        }
    }
    ...
    private void appendEdgePath(ShapeAppearancePathSpec shapeAppearancePathSpec, int i) {
        ...
        if (!this.edgeIntersectionCheckEnabled || VERSION.SDK_INT < 19 || (!edgeTreatmentForIndex.forceIntersection() && !pathOverlapsCorner(path, i) && !pathOverlapsCorner(path, i2))) {
            this.shapePath.applyToPath(this.edgeTransforms[i], shapeAppearancePathSpec.path);
        } else {
            path.op(path, this.boundsPath, Op.DIFFERENCE);
