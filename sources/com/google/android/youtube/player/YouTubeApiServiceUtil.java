package com.google.android.youtube.player;

import android.content.Context;
import android.content.pm.PackageInfo;
import android.content.pm.PackageManager;
import android.content.pm.PackageManager.NameNotFoundException;
import android.util.Base64;
import com.google.android.youtube.player.internal.C2838z;
import java.io.ByteArrayInputStream;
import java.security.cert.CertificateException;
import java.security.cert.CertificateFactory;
import java.security.cert.X509Certificate;
import java.util.Arrays;

public final class YouTubeApiServiceUtil {

    /* renamed from: a */
    private static final byte[][] f1615a = {Base64.decode("", 0), Base64.decode("", 0), Base64.decode("", 0), Base64.decode("", 0), Base64.decode("", 0), Base64.decode("", 0)};

    private YouTubeApiServiceUtil() {
    }

    /* renamed from: a */
    private static boolean m1426a(PackageInfo packageInfo) {
        try {
            ((X509Certificate) CertificateFactory.getInstance("X509").generateCertificate(new ByteArrayInputStream(packageInfo.signatures[0].toByteArray()))).checkValidity();
            if (packageInfo.signatures.length != 1) {
                return false;
            }
            byte[] byteArray = packageInfo.signatures[0].toByteArray();
            int i = 0;
            while (true) {
                byte[][] bArr = f1615a;
                if (i >= bArr.length) {
                    break;
                } else if (Arrays.equals(bArr[i], byteArray)) {
                    return true;
                } else {
                    i++;
                }
            }
            return false;
        } catch (CertificateException unused) {
        }
    }

    public static YouTubeInitializationResult isYouTubeApiServiceAvailable(Context context) {
        try {
            PackageManager packageManager = context.getPackageManager();
            String a = C2838z.m1750a(context);
            return !m1426a(packageManager.getPackageInfo(a, 64)) ? YouTubeInitializationResult.SERVICE_INVALID : C2838z.m1751a(context, a) ? YouTubeInitializationResult.SERVICE_VERSION_UPDATE_REQUIRED : !packageManager.getApplicationInfo(a, 0).enabled ? YouTubeInitializationResult.SERVICE_DISABLED : YouTubeInitializationResult.SUCCESS;
        } catch (NameNotFoundException unused) {
            return YouTubeInitializationResult.SERVICE_MISSING;
        }
    }
}
