...
public final class AdsMediaSource extends CompositeMediaSource<MediaPeriodId> {
    ...
    public MediaPeriod createPeriod(MediaPeriodId mediaPeriodId, Allocator allocator, long j) {
        ...
        List list = (List) this.maskingMediaPeriodByAdMediaSource.get(mediaSource2);
        if (list == null) {
            maskingMediaPeriod2.createPeriod(new MediaPeriodId(((Timeline) Assertions.checkNotNull(this.adGroupTimelines[i][i2])).getUidOfPeriod(0), mediaPeriodId2.windowSequenceNumber));
        } else {
    ...
    public void releasePeriod(MediaPeriod mediaPeriod) {
        ...
        List list = (List) this.maskingMediaPeriodByAdMediaSource.get(maskingMediaPeriod.mediaSource);
        if (list != null) {
    ...
    /* access modifiers changed from: protected */
    public void onChildSourceInfoRefreshed(MediaPeriodId mediaPeriodId, MediaSource mediaSource, Timeline timeline) {
    ...
    private void onAdSourceInfoRefreshed(MediaSource mediaSource, int i, int i2, Timeline timeline) {
        ...
        List list = (List) this.maskingMediaPeriodByAdMediaSource.remove(mediaSource);
        if (list != null) {
            Object uidOfPeriod = timeline.getUidOfPeriod(0);
            for (int i3 = 0; i3 < list.size(); i3++) {
                MaskingMediaPeriod maskingMediaPeriod = (MaskingMediaPeriod) list.get(i3);
                maskingMediaPeriod.createPeriod(new MediaPeriodId(uidOfPeriod, maskingMediaPeriod.f1493id.windowSequenceNumber));
            }
        }
        maybeUpdateSourceInfo();
    }
