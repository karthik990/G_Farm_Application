...
public final class GaplessInfoHolder {
    ...
    public boolean setFromMetadata(Metadata metadata) {
        for (int i = 0; i < metadata.length(); i++) {
            ...
            boolean z = entry instanceof CommentFrame;
            String str = GAPLESS_DESCRIPTION;
            if (z) {
                CommentFrame commentFrame = (CommentFrame) entry;
                if (str.equals(commentFrame.description) && setFromComment(commentFrame.text)) {
                    return true;
                }
            } else if (entry instanceof InternalFrame) {
                InternalFrame internalFrame = (InternalFrame) entry;
                if (GAPLESS_DOMAIN.equals(internalFrame.domain) && str.equals(internalFrame.description) && setFromComment(internalFrame.text)) {
                    return true;
                }
            } else {
                continue;
