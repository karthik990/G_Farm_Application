...
public class MaxCore {
    ...
    public Request sortRequest(Request request) {
        ...
        List findLeaves = findLeaves(request);
    ...
    private Request constructLeafRequest(List<Description> list) {
        ...
        return new Request() {
            public Runner getRunner() {
                try {
                    return new Suite(null, arrayList) {
                    };
    ...
    private Runner buildRunner(Description description) {
        ...
        if (description.toString().startsWith(MALFORMED_JUNIT_3_TEST_CLASS_PREFIX)) {
            return new JUnit38ClassRunner((Test) new TestSuite(getMalformedTestClass(description)));
        }
        ...
        sb.append("Can't build a runner from description [");
    ...
    private void findLeaves(Description description, Description description2, List<Description> list) {
        if (!description2.getChildren().isEmpty()) {
            ...
            while (it.hasNext()) {
                findLeaves(description2, (Description) it.next(), list);
            }
        } else if (description2.toString().equals("warning(junit.framework.TestSuite$1)")) {
            StringBuilder sb = new StringBuilder();
            sb.append(MALFORMED_JUNIT_3_TEST_CLASS_PREFIX);
            sb.append(description);
            list.add(Description.createSuiteDescription(sb.toString(), new Annotation[0]));
        } else {
